...
private lemma runsOf_cons_pos {p : α → Bool} {c : α} (hc : p c = true) (t : Text α) :
...
private lemma runsOf_cons_neg {p : α → Bool} {c : α} (hc : p c = false) (t : Text α) :
...
private lemma runsOf_eq_tail_filter {p : α → Bool} {t : Text α}
...
private lemma runsOf_eq_cons_tail_filter {p : α → Bool} {t : Text α}
    (h : (t.splitOnP p).headD [] ≠ []) :
...
private lemma ne_nil_of_mem_runsOf {p : α → Bool} {t w : Text α} (h : w ∈ runsOf p t) :
    w ≠ [] := by
...
private lemma flatten_runsOf (p : α → Bool) (t : Text α) :
...
private lemma length_le_length_flatten (p : α → Bool) (t : Text α) :
    (runsOf p t).length ≤ (runsOf p t).flatten.length := by
  have : ∀ l : List (Text α), ([] ∉ l) → l.length ≤ l.flatten.length := by
    intro l
    induction l with
    | nil => simp
    | cons w l ih =>
...
private lemma headD_splitOnP_nil (p : α → Bool) :
    ((([] : Text α)).splitOnP p).headD [] = [] := by
  simp [List.splitOnP_nil]

private lemma headD_splitOnP_cons (p : α → Bool) (c : α) (t : Text α) :
...
private lemma length_runsOf_cons_pos {p : α → Bool} {c : α} (hc : p c = true) (t : Text α) :
    (runsOf p (c :: t)).length = (runsOf p t).length := by
  rw [runsOf_cons_pos hc]

private lemma length_runsOf_cons_neg {p : α → Bool} {c : α} (hc : p c = false) (t : Text α) :
    (runsOf p (c :: t)).length =
...
private lemma length_runsOf_le (p : α → Bool) (t : Text α) :
  ...
  | cons c t ih =>
    ...
    · have hq : (fun c => !p c) c = false := by simp [hc]
      rw [length_runsOf_cons_pos hc, length_runsOf_cons_neg (p := fun c => !p c) hq,
    ...
    · have hcf : p c = false := by simpa using hc
      ...
      rw [if_neg (show ¬ ((c :: t).splitOnP p).headD [] = [] by
        ...
        length_runsOf_cons_neg hcf, length_runsOf_cons_pos (p := fun c => !p c) hq]
      by_cases h : (t.splitOnP p).headD [] = []
      · rw [if_pos h]; rw [if_pos h] at ih; omega
      · rw [if_neg h]; rw [if_neg h] at ih; omega

variable [DecidableEq α] [Alphabet α]

private lemma words_eq_runsOf (t : Text α) :
    words t = runsOf (fun c => decide (IsSeparator c)) t :=
  rfl

private lemma breaks_eq_runsOf (t : Text α) : breaks t = runsOf (fun c => decide (IsLetter c)) t :=
  rfl

/-- The two predicates are complementary, which is what couples `words` to
`breaks`. -/
private lemma letterP_eq (c : α) :
    decide (IsLetter c) = !decide (IsSeparator c) := by
  simp [IsLetter]

/-- The characters of a text are its letters and its separators. -/
private lemma length_flatten_words_add_length_flatten_breaks (t : Text α) :
...
@[simp] private lemma words_nil : words ([] : Text α) = [] := by simp [words_eq_runsOf]

@[simp] private lemma breaks_nil : breaks ([] : Text α) = [] := by simp [breaks_eq_runsOf]

/-- A separator ends a word. -/
private lemma words_cons_sep {c : α} (hc : IsSeparator c) (t : Text α) :
...
private lemma words_of_forall_sep {b : Text α} (h : ∀ c ∈ b, IsSeparator c) :
    words b = [] := by
  induction b with
  | nil => simp
  | cons c t ih =>
...
lemma words_of_forall_letter {w : Text α} (hne : w ≠ []) (h : ∀ c ∈ w, IsLetter c) :
    words w = [w] := by
...
one equation. -/
lemma words_append_sep {c : α} (hc : IsSeparator c) (x y : Text α) :
...
private lemma words_append_break {b : Text α} (hb : b ∈ Break) :
    ∀ x y : Text α, words (x ++ b ++ y) = words x ++ words y := by
  obtain ⟨hne, hsep⟩ := hb
  induction b with
...
private lemma words_eq_of_recast1 {i o : Text α} (h : Recast1 i o) : words i = words o := by
  obtain ⟨b, hb, h⟩ := h
...
theorem words_eq_of_recast {i o : Text α} (h : Recast i o) : words o = words i := by
  induction h with
  | refl => rfl
...
versa. -/
private lemma length_words_le (t : Text α) : (words t).length ≤ (breaks t).length + 1 := by
...
private lemma length_breaks_le (t : Text α) : (breaks t).length ≤ (words t).length + 1 := by
...
theorem length_breaks_ge {i o : Text α} (h : Recast i o) :
...
lemma length_add_length_words_le (t : Text α) :
...
lemma le_length_of_recast {i o : Text α} (h : Recast i o) :
