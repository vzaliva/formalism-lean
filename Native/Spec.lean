...
Nothing here is an implementation.  `Layout` is a property of a list of lines,
`ByLayout` and `ByText` are relations, and the relation is not a function:
`Native.Properties` shows an input with two outputs, as both of Meyer's texts do
for theirs.
